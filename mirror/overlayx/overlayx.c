...
int overlay_init(const char *img_path)
{
    ...
    vignette_res = vc_dispmanx_resource_create(IMAGE_FORMAT, screen_width,
                                               screen_height, &vc_image_ptr);

    fade_res = vc_dispmanx_resource_create(IMAGE_FORMAT, screen_width,
                                           screen_height, &vc_image_ptr);
    if (!vignette_res || !fade_res)
    {
    ...
    {
        fprintf(stderr, "Vignette image size is not the same as the screen size");
        return 1;
    }
    ...
    fade_elmt = vc_dispmanx_element_add(update, display, FADE_LAYER, &dst_rect, fade_res,
